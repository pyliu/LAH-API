...
switch ($_POST["type"]) {
	case "user_message":
		Logger::getInstance()->info("XHR [user_message] 查詢使用者信差訊息【".$_POST["id"].", ".$_POST["name"].", ".$_POST["ip"].", ".$_POST["count"]."】請求");
		...
		if (empty($results)) {
			Logger::getInstance()->info("XHR [user_message] 查無 ${param} 信差訊息。");
	...
	case "user_unread_message":
		$param = $_POST["id"] ?? $_POST["name"] ?? $_POST["ip"];
		Logger::getInstance()->info("XHR [user_unread_message] 查詢使用者未讀信差訊息【".$param."】請求");
		$param = empty($param) ? $client_ip : $param;
		$message = new Message();
		...
		if (empty($results)) {
			Logger::getInstance()->info("XHR [user_unread_message] 查無 ${param} 未讀信差訊息。");
	...
	case "set_read_user_message":
		Logger::getInstance()->info("XHR [set_read_user_message] 設定已讀使用者信差訊息【".$_POST["sn"]."】請求");
		$message = new Message();
		...
		if ($result) {
			...
			Logger::getInstance()->error("XHR [set_read_user_message] "."設定 ".$_POST['sn']." 已讀信差訊息失敗。");
			echoJSONResponse(print_r($message->lastError(), true));
		}
		break;
	case "set_unread_user_message":
		Logger::getInstance()->info("XHR [set_unread_user_message] 設定未讀使用者信差訊息【".$_POST["sn"]."】請求");
		$message = new Message();
		...
		if ($result) {
			...
			Logger::getInstance()->error("XHR [set_unread_user_message] "."設定 ".$_POST['sn']." 未讀信差訊息失敗。");
			echoJSONResponse(print_r($message->lastError(), true));
		}
		break;
	case "del_user_message":
		Logger::getInstance()->info("XHR [del_user_message] 刪除訊息【".$_POST["sn"]."】請求");
		$msg = new Message();
		...
		if ($ret) {
			...
			Logger::getInstance()->error("XHR [del_user_message] 刪除「".$_POST["sn"]."」訊息失敗。");
			echoJSONResponse("刪除「".$_POST["sn"]."」訊息失敗");
		}
		break;
	case "send_message":
		Logger::getInstance()->info("XHR [send_message] 送出訊息【".$_POST["title"].", ".$_POST["content"].", ".$_POST["who"].", ".$_POST["send_time"].", ".$_POST["end_time"]."】請求");
		$msg = new Message();
		...
		if ($id > 0) {
			Logger::getInstance()->info("XHR [send_message] 給「".$_POST["who"]."」訊息「".$_POST["title"]."」已寫入內網資料庫【sn: $id 】");
			...
			Logger::getInstance()->warning("XHR [send_message] ${msg}");
			...
			Logger::getInstance()->error("XHR [send_message] 新增「".$_POST["title"]."」訊息失敗【${id}】。");
			echoJSONResponse("新增 ".$_POST["title"]." 訊息失敗【${id}】。");
		}
		break;
    case "upd_ext_doc":
        Logger::getInstance()->info("XHR [upd_ext_doc] 更新舊資料庫分機號碼【".$_POST["id"].", ".$_POST["ext"]."】請求");
        ...
        if ($result) {
            ...
            Logger::getInstance()->info("XHR [upd_ext_doc] ".$msg);
            echoJSONResponse($msg, STATUS_CODE::SUCCESS_NORMAL);
        } else {
            Logger::getInstance()->info("XHR [upd_ext_doc] 更新舊資料庫分機號碼 ".$_POST["id"].", ".$_POST["ext"]." 失敗。");
            echoJSONResponse("更新舊資料庫分機號碼 ".$_POST["id"].", ".$_POST["ext"]." 失敗。");
        }
        break;
	case "upd_ip_tdoc":
		Logger::getInstance()->info("XHR [upd_ip_tdoc] 設定 ".$_POST['id']." 為 ".$_POST['ip']."請求");
		$mssql_on = $system->isMSSQLEnable();
		if ($mssql_on) {
			...
			// if ($result) {
				...
				Logger::getInstance()->info("XHR [upd_ip_tdoc] ".$msg);
				echoJSONResponse($msg, STATUS_CODE::SUCCESS_NORMAL);
			// } else {
			...
			Logger::getInstance()->info("XHR [upd_ip_tdoc] MSSQL連線未啟用。");
