...
switch ($_POST["type"]) {
    ...
    case "stats_reg_cert_case":
        Logger::getInstance()->info("XHR [stats_reg_cert_case] 取得臨櫃謄本核發案件(".$_POST['st']." ~ ".$_POST['ed'].")請求。");
        ...
        if ($arr) {
            $count = count($arr);
            Logger::getInstance()->info("XHR [stats_reg_cert_case] 取得臨櫃謄本核發案件(".$_POST['st']." ~ ".$_POST['ed'].")成功。 ($count)");
            echoJSONResponse("取得 $count 筆資料。", STATUS_CODE::SUCCESS_NORMAL, array(
                "raw" => $arr
            ));
        } else {
            $err = "取得臨櫃謄本核發案件資料失敗。 ".$_POST['st']." ~ ".$_POST['ed'];
    ...
    case "stats_latest_ap_conn":
        $apHistory = new SQLiteAPConnectionHistory($_POST["ap_ip"]);
        $arr = $apHistory->getLatest($_POST["all"]);
        if ($arr !== false) {
            $count = count($arr);
            echoJSONResponse("取得 $count 筆資料。", STATUS_CODE::SUCCESS_NORMAL, array(
                "raw" => $arr
    ...
    case "stats_reg_initail_review_count":
        ...
        break;
    case "stats_reg_final_review_count":
        $arr = $mock ? $cache->get('stats_reg_final_review_count') : $stats->getFinalReviewCaseStats($_POST["st"], $_POST["ed"]);
        $cache->set('stats_reg_final_review_count', $arr);
        if (is_array($arr)) {
            $count = count($arr);
            echoJSONResponse("取得 $count 筆資料。", STATUS_CODE::SUCCESS_NORMAL, array(
                "raw" => $arr
            ...
            $error = "取得複審案件統計資料 ".$_POST["st"]." ~ ".$_POST["ed"]." 失敗。";
            Logger::getInstance()->error("XHR [stats_reg_final_review_count] $error");
            echoJSONResponse($error);
        }
        break;
    case "stats_reg_chief_review_count":
        $arr = $mock ? $cache->get('stats_reg_chief_review_count') : $stats->getChiefReviewCaseStats($_POST["st"], $_POST["ed"]);
        $cache->set('stats_reg_chief_review_count', $arr);
        if (is_array($arr)) {
            $count = count($arr);
            echoJSONResponse("取得 $count 筆資料。", STATUS_CODE::SUCCESS_NORMAL, array(
                "raw" => $arr
            ...
            $error = "取得課長案件統計資料 ".$_POST["st"]." ~ ".$_POST["ed"]." 失敗。";
            Logger::getInstance()->error("XHR [stats_reg_chief_review_count] $error");
            echoJSONResponse($error);
        }
        break;
    case "stats_reg_owner_data":
        $birth_year = $this_year - 100;
        if ($_POST['birth_year'] > 0) {
            $birth_year = intval($_POST['birth_year']);
        }
        Logger::getInstance()->info("XHR [stats_reg_owner_data] 準備取得所有權人 ".$_POST["owner_type"]." 資料，出生年：".$birth_year." ...");
        $arr = $mock ? $cache->get('stats_reg_owner_data') : $stats->getHundredYearsOwnerData($birth_year, $_POST['owner_type']);
        $cache->set('stats_reg_owner_data', $arr);
        if (is_array($arr)) {
            $count = count($arr);
            Logger::getInstance()->info("XHR [stats_reg_owner_data] 取得 $count 筆 ".$_POST["owner_type"]." 所有權人資料成功。");
            echoJSONResponse("取得 $count 筆 ".$_POST["owner_type"]." 資料。", STATUS_CODE::SUCCESS_NORMAL, array(
                "raw" => $arr
            ));
        } else {
            $error = "取得 ".$birth_year." 出生所有權人 ".$_POST["owner_type"]." 資料失敗。";
            Logger::getInstance()->error("XHR [stats_reg_owner_data] $error");
            echoJSONResponse($error);
        }
        break;
	default:
