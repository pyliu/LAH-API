                    ...
                    this.$http.post(CONFIG.API.JSON.STATS, {
                        ...
                        if (res.data.status == XHR_STATUS_CODE.SUCCESS_NORMAL) {
                            if (res.data.data_count == 0) {
                                this.notify({title: '跨所 AP H0 歷史連線數', message: '無資料，無法繪製圖形', type: 'warning'});
                            } else {
                            ...
                            this.alert({title: `取得跨所 AP H0 歷史連線數`, message: `取得跨所 AP H0 歷史連線數回傳狀態碼有問題【${res.data.status}】`, variant: "warning"});
                        }
                    }).catch(err => {
                        this.error = err;
                    }).finally(() => {
