    ...
    Vue.component('lah-xap-connection-chart', {
        template: `<lah-transition appear>
            <b-card border-variant="secondary" class="shadow" v-b-visible="visible">
                <lah-chart ref="chart" :label="label" :items="items" :type="type" :bg-color="bg_color" :title="title" title-pos="left" @click="history" :aspect-ratio="aspectRatio" :title-font-size="24" :label-font-size="18"></lah-chart>
            ...
            request() {
                ...
                this.$http.post(CONFIG.API.JSON.STATS, {
                    ...
                    if (res.data.status == XHR_STATUS_CODE.SUCCESS_NORMAL) {
                        ...
                        this.$warn(`取得${this.ip}連線數AP連線數回傳狀態碼有問題【${res.data.status}】`, res.data);
                    }
                }).catch(err => {
                    this.error = err;
                }).finally(() => {
