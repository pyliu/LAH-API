if (Vue) {
        ...
        computed: {
            ...
            links() {
                return [{
                    ...
                    text: "監控看板",
                    url: `http://${location.host}:8080/inf/dashboard`,
                    icon: "desktop",
                    need_admin: true,
                    children: [{
                        text: "跨所AP看板",
            ...
            <b-card no-body v-if="found">
                <b-tabs card :end="useEndTabs" :pills="useEndTabs" :small="useEndTabs" fill>
                    ...
                    <b-tab v-if="foundCount == 1 && !foundLeft && message_ui">
                        <template v-slot:title>
                            <lah-fa-icon icon="comment-dots" prefix="far"> 私訊</lah-fa-icon>
                        </template>
    ...
    Vue.component("lah-sur-case-dialog", {
        template: `<div>
            收件字號：<a title="案件辦理情形 on ${CONFIG.AP_SVR}" href="javascript:void(0)" @click="openNewWindow('http://' + CONFIG.AP_SVR + ':9080/LandHA/Dispatcher?REQ=CMC0202&GRP=CAS&MM01=' + json.raw['MM01'] + '&MM02=' + json.raw['MM02'] + '&MM03=' + json.raw['MM03'] + '&RM90=', $event)">{{json.收件字號}}</a> </br>
            收件時間：{{json.收件時間}} <br/>
