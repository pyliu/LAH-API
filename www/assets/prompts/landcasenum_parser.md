...
- **案件號**：數字，正式格式為 6 碼並只在不足 6 碼情況下前面補零。  
...
- **案件號**
  - 案件號正式格式固定為 **6 碼**。
  - 若輸入為 1～5 碼數字，僅允許於左側補 0 至滿 6 碼。
  - 若輸入本身已為 6 碼，**禁止補零、禁止截斷、禁止修改任何數字**，必須原樣使用。
  - **case_no 長度最終必須等於 6**，不得為 5 碼、7 碼或其他長度。
  - normalized 最後一段必須與 case_no 完全一致。
...
1. case_no 長度必須 == 6。
2. normalized 最後一段必須 == case_no。
3. 不得產生任何 7 碼案件號。
4. 若輸入本身為 6 碼：
   - case_no 必須與輸入完全相同。
   - 禁止增加前導 0。
   - 禁止刪除任何數字。
5. 若驗證失敗，必須重新修正後再輸出 JSON，不可直接輸出錯誤格式。
...
### 最終輸出驗證（Output Validation）

在輸出 JSON 前逐筆檢查：

- case_no 是否恰為 6 碼。
- normalized 最後一段是否與 case_no 完全一致。
- 是否存在任何長度不是 6 的 case_no。

若有任一筆不符合，必須修正後重新產生 JSON，不得直接輸出。
...
- **全形數字**：需轉半形再處理。  
- **案件號補零規則**
  - 僅當案件號長度 < 6 時，才允許左側補 0。
  - 若案件號長度 = 6，禁止任何補零。
  - 最終 case_no 必須固定為 6 碼。
  - 範例：5→000005、25→000025、25600→025600、112440→112440（禁止變成0112440）、999999→999999。  
