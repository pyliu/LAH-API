...
class Query {
	...
	private function getCrossCountyCodeMap() {
		return array(
				'A1' => array('AA', '古亭'),
				'A2' => array('AB', '建成'),
				'A3' => array('AC', '中山'),
    ...
    $this->db_wrapper->getDB()->bind(":bv_rm02_code", $code);
		...
		if (endsWith($code, $ty_hold_site_code)) {
			$map = $this->getCrossCountyCodeMap();
	...
	public function insertLocalCRCLD($l3_crcld) {
	...
	public function updateLocalCRCLD($l3_crcld) {
		if (is_array($l3_crcld)) {
			$year = $l3_crcld['CL01'];
			$code = $l3_crcld['CL02'];
			$num = $l3_crcld['CL03'];
	...
	public function getXCaseCRCRD($crcld, $office_code) {
		if (!$this->db_wrapper->reachable()) {
			return -1;
		}
      ...
      return -2;
		...
		$this->db_wrapper->getDB()->parse("
			...
			WHERE 1=1
				...
				and RC06 = :bv_rc06
		");
		$this->db_wrapper->getDB()->bind(":bv_rc01", $rc04);
		$this->db_wrapper->getDB()->bind(":bv_rc04", $rc01);
		$this->db_wrapper->getDB()->bind(":bv_rc06", $rc06);
		$this->db_wrapper->getDB()->execute();
		$remote_row = $this->db_wrapper->getDB()->fetch(true);
	...
	public function getLocalCRCRD($id) {
		if (!$this->db_wrapper->reachable()) {
			return -1;
		}
	...
	public function insertLocalCRCRD($l3_crcrd) {
		if (is_array($l3_crcld)) {
			$year = $l3_crcld['CL01'];
			$code = $l3_crcld['CL02'];
			$num = $l3_crcld['CL03'];
	...
	public function updateLocalCRCRD($l3_crcrd) {
		if (is_array($l3_crcld)) {
			$year = $l3_crcld['CL01'];
			$code = $l3_crcld['CL02'];
			$num = $l3_crcld['CL03'];
	...
	public function syncXCaseFixData($id) {
		...
		// L1HX0H03 has link data of the case
		if (is_array($l3_crcld)) {
			// 檢查本地 CRCLD 並更新資料
