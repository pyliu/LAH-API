...
class RegCaseData {
    ...
    public function getBakedData() {
        ...
        $ret = array(
            ...
            "權利人姓名" => empty($row["RM19"]) ? "" : preg_replace("/BAD\+[[:alpha:][:digit:]]{3}/", "○", $row["RM19"]),
            "義務人統編" => empty($row["RM21"]) ? "" : $row["RM21"],
            "義務人姓名" => empty($row["RM22"]) ? "" : preg_replace("/BAD\+[[:alpha:][:digit:]]{3}/", "○", $row["RM22"]),
            "義務人人數" => empty($row["RM23"]) ? "" : $row["RM23"],
            "手機號碼" => empty($row["RM102"]) ? "" : $row["RM102"],
            "代理人統編" => empty($row["RM24"]) ? "" : $row["RM24"],
            "代理人姓名" => empty($row["AB02"]) ? "" : preg_replace("/BAD\+[[:alpha:][:digit:]]{3}/", "○", $row["AB02"]),
            "代理人住址" => empty($row["AB03"]) ? "" : preg_replace("/BAD\+[[:alpha:][:digit:]]{3}/", "○", $row["AB03"]),
