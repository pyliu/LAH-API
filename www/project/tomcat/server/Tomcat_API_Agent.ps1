...
# -------------------------------------------------------------------------
# 自動服務守護：喚醒「啟動類型=自動」但已停止的服務 (預設針對 SQL Server)
# -------------------------------------------------------------------------
function Watch-AutoServices {
    if (-not $enableWatchAutoServices) { return }

    $watchLabel = "[服務守護]"
    $mode = if ($watchAutoServiceNames -and $watchAutoServiceNames.Count -gt 0) { "A" } else { "B" }
    Write-ApiLog "--- $watchLabel 開始掃描 (模式 $mode) ---" -Color DarkCyan

    try {
        $targetServices = @()

        if ($watchAutoServiceNames -and $watchAutoServiceNames.Count -gt 0) {
            # 模式 A：只檢查 .env 指定的服務清單
            Write-ApiLog " -> $watchLabel 指定清單: $($watchAutoServiceNames -join ', ')" -Color DarkCyan
            foreach ($svcName in $watchAutoServiceNames) {
                ...
                if ($null -ne $svc) {
                    $targetServices += $svc
                    Write-ApiLog " -> $watchLabel 清單項目 [$svcName] -> 搜尋到: $($svc.DisplayName) ($($svc.Name))" -Color DarkGray
                } else {
                    Write-ApiLog " -> $watchLabel !!! 找不到服務: [$svcName]，請確認名稱是否正確" -Color Yellow
                }
            }
        } else {
            # 模式 B：自動掃描所有啟動類型=自動但已停止的服務
            Write-ApiLog " -> $watchLabel 開始自動掃描啟動類型=Auto 但已停止的服務..." -Color DarkCyan
            ...
            Write-ApiLog " -> $watchLabel 掃描完成，找到 $($targetServices.Count) 個待處理的服務" -Color DarkCyan
        }

        if ($targetServices.Count -eq 0) {
            Write-ApiLog " -> $watchLabel 所有服務狀態正常，無需處理" -Color DarkGray
        }
        ...
        foreach ($svc in $targetServices) {
            ...
            try {
                ...
                $startMode = if ($wmiSvc) { $wmiSvc.StartMode } else { '未知' }
            } catch { $startMode = '查詢失敗' }

            $statusColor = if ($svc.Status -eq 'Running') { 'Green' } else { 'Red' }
            Write-ApiLog " -> $watchLabel 檢查: [$($svc.Name)]  狀態=$($svc.Status)  啟動類型=$startMode" -Color $statusColor

            $isAutoStart = ($startMode -eq 'Auto') -or ($watchAutoServiceNames.Count -gt 0)

            if ($isAutoStart -and $svc.Status -ne 'Running') {
                Write-ApiLog "!!! $watchLabel 偵測到「$($svc.DisplayName)」($($svc.Name)) 啟動類型為自動但已停止，嘗試喚醒..." -Color Yellow
                try {
                    Start-Service -Name $svc.Name -ErrorAction Stop
                    Write-ApiLog " -> $watchLabel Start-Service 指令已發出，等待啟動中...(max 30s)" -Color DarkYellow
                    $waited = 0
                    do {
                        Start-Sleep -Seconds 2
                        $waited += 2
                        $svc.Refresh()
                        Write-ApiLog " -> $watchLabel   已等待 ${waited}s，目前狀態=$($svc.Status)" -Color DarkGray
                    } while ($svc.Status -ne 'Running' -and $waited -lt 30)

                    if ($svc.Status -eq 'Running') {
                        Write-ApiLog ">>> $watchLabel 「$($svc.DisplayName)」已成功喚醒並恢復執行中！" -Color Green
                        if ($enableAdminNotifications) {
                            Send-SysAdminNotify `
                                -title "服務自動喚醒通知" `
                                -content "Agent 偵測到「$($svc.DisplayName)」($($svc.Name)) 已停止，已自動將其喚醒並恢復執行。"
                        }
                    } else {
                        Write-ApiLog "!!! $watchLabel 「$($svc.DisplayName)」喚醒後 30 秒內仍未達到 Running 狀態，請手動介入檢查" -Color Red
                        if ($enableAdminNotifications) {
                            Send-SysAdminNotify `
                                -title "服務喚醒失敗警示" `
                                -content "Agent 嘗試喚醒「$($svc.DisplayName)」($($svc.Name))，30 秒後狀態仍為 $($svc.Status)，請手動介入檢查。"
                        }
                    }
                } catch {
                    Write-ApiLog "!!! $watchLabel 喚醒「$($svc.DisplayName)」失敗: $($_.Exception.Message)" -Color Red
                    if ($enableAdminNotifications) {
                        Send-SysAdminNotify `
                            -title "服務喚醒失敗警示" `
                            -content "Agent 嘗試喚醒「$($svc.DisplayName)」($($svc.Name)) 時發生錯誤: $($_.Exception.Message)"
                    }
                }
            } else {
                Write-ApiLog " -> $watchLabel   「$($svc.DisplayName)」 狀態正常，無需處理" -Color DarkGray
            }
        }
    } catch {
        Write-ApiLog "!!! $watchLabel 掃描服務時發生例外: $($_.Exception.Message)" -Color Red
    }

    Write-ApiLog "--- $watchLabel 掃描完成 ---" -Color DarkCyan
}
