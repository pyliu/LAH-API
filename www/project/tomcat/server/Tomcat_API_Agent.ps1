...
.HISTORY
    ...
    版本 v2.2
    - 服務守護時段限制：Watch-AutoServices 僅於每日 08:00~17:00 工作時段內執行，
      避免非工作時間頻繁觸發系統喚醒操作，降低夜間誤報風險。
    - 強化服務守護診斷日誌：每次掃描週期均輸出完整執行狀態，
      包含模式、指定清單、各服務狀態與啟動類型，便於追蹤是否正確執行。

    版本 v2.3 - [目前版本]
    - 強化服務守護 (Watch-AutoServices) 診斷日誌：
      1. 增加掃描週期的起訖標記。
      2. 智慧判斷模式：指定清單模式下，逐一輸出名單內服務的「名稱、狀態、啟動類型」；
         全局掃描模式下，導入防洪機制，全數正常時僅輸出單筆健康提示，避免日誌洗版。
      3. 增強防呆：若指定清單中的服務名稱在系統中不存在，會印出 Warning 提示。
...
# -- SQL Server (未啟動保護設定) --------------------------------------------
# WATCH_AUTO_SERVICES      : 是否啟用自動服務守護 (true/false，預設 true)
# WATCH_AUTO_SERVICE_NAMES : 逗號分隔的服務名稱清單；
...
$global:LastServiceWatchTime = [datetime]::MinValue
$restartScript = $false
...
# -------------------------------------------------------------------------
# 自動服務守護：喚醒「啟動類型=自動」但已停止的服務 (具備 v2.3 增強日誌)
# -------------------------------------------------------------------------
function Watch-AutoServices {
    if (-not $enableWatchAutoServices) { return }

    try {
        Write-ApiLog " -> [服務守護] 開始執行服務狀態掃描週期..." -Color DarkGray
        
        $targetServices = @()
        $isModeA = ($watchAutoServiceNames -and $watchAutoServiceNames.Count -gt 0)

        if ($isModeA) {
            $modeDesc = "指定清單模式 ($($watchAutoServiceNames -join ', '))"
            Write-ApiLog " -> [服務守護] 掃描模式: $modeDesc" -Color DarkGray
            
            # 模式 A：只檢查 .env 指定的服務清單
            foreach ($svcName in $watchAutoServiceNames) {
                $svcName = $svcName.Trim()
                if ($svcName -eq '') { continue }
                
                $svc = Get-Service -Name $svcName -ErrorAction SilentlyContinue
                if ($null -eq $svc) {
                    $svc = Get-Service -ErrorAction SilentlyContinue |
                           Where-Object { $_.DisplayName -eq $svcName } |
                ...
                if ($null -ne $svc) { 
                    ...
                    Write-ApiLog "    - 警告: 找不到指定的系統服務 [$svcName]，請檢查設定檔。" -Color DarkYellow
                }
            }
            ...
            foreach ($svc in $targetServices) {
                ...
                try {
                    ...
                    $startMode = if ($wmiSvc) { $wmiSvc.StartMode } else { '' }
                } catch {}

                Write-ApiLog "    - 檢查服務: $($svc.DisplayName) [$($svc.Name)] | 狀態: $($svc.Status) | 啟動類型: $startMode" -Color DarkGray

                $isAutoStart = ($startMode -eq 'Auto') -or $true # 在清單內的必定檢查
                if ($isAutoStart -and $svc.Status -ne 'Running') {
                    Wake-TargetService -svc $svc
                }
            }
            
        } else {
            Write-ApiLog " -> [服務守護] 掃描模式: 全局掃描模式 (所有啟動類型為自動的服務)" -Color DarkGray
            ...
            # 模式 B 狀態輸出：防洪機制，避免列出上百個服務
            if ($targetServices.Count -eq 0) {
                Write-ApiLog "    - 系統服務健康檢查: 所有自動啟動服務皆正常運行中。" -Color DarkGray
            } else {
                Write-ApiLog "    - 警告: 發現 $($targetServices.Count) 個自動啟動服務處於異常停止狀態！" -Color Yellow
                foreach ($svc in $targetServices) {
                    Write-ApiLog "    - 異常服務: $($svc.DisplayName) [$($svc.Name)] | 狀態: $($svc.Status)" -Color Yellow
                    Wake-TargetService -svc $svc
                }
            }
        }
        
        Write-ApiLog " -> [服務守護] 掃描週期完成。" -Color DarkGray

    } catch {
        Write-ApiLog "!!! [服務守護] 掃描服務時發生例外: $($_.Exception.Message)" -Color Red
    }
}

# 將喚醒邏輯獨立為內部子函數以保持主程式碼整潔
function Wake-TargetService {
    param($svc)
    Write-ApiLog "!!! [服務保護] 偵測到服務「$($svc.DisplayName)」($($svc.Name)) 異常停止，嘗試喚醒..." -Color Yellow
    try {
        ...
        if ($svc.Status -eq 'Running') {
            Write-ApiLog ">>> [服務保護] 服務「$($svc.DisplayName)」已成功喚醒並恢復執行中。" -Color Green
            if ($enableAdminNotifications) {
                Send-SysAdminNotify -title "服務自動喚醒通知" -content "Agent 偵測到服務「$($svc.DisplayName)」($($svc.Name)) 已停止，已自動將其喚醒並恢復執行。"
            }
        } else {
            Write-ApiLog "!!! [服務保護] 服務「$($svc.DisplayName)」喚醒後 30 秒內仍未達到 Running 狀態。" -Color Red
            if ($enableAdminNotifications) {
                Send-SysAdminNotify -title "服務喚醒失敗警示" -content "Agent 嘗試喚醒服務「$($svc.DisplayName)」($($svc.Name))，等候 30 秒後狀態仍為 $($svc.Status)，請登入系統檢查。"
            }
        }
    } catch {
        Write-ApiLog "!!! [服務保護] 喚醒服務「$($svc.DisplayName)」失敗: $($_.Exception.Message)" -Color Red
        if ($enableAdminNotifications) {
            Send-SysAdminNotify -title "服務喚醒失敗警示" -content "Agent 嘗試喚醒服務「$($svc.DisplayName)」($($svc.Name)) 時發生錯誤: $($_.Exception.Message)"
        }
    }
}
...
Write-ApiLog " Tomcat API Agent v2.3 (Port: $port) " -Color Cyan
Write-ApiLog "----------------------------------------" -Color Cyan
...
try {
    ...
    Write-ApiLog "!!! 無法綁定 Port $port，程序終止。" -Color Red
...
while ($listener.IsListening) {
    try {
        ...
        # 服務守護：每隔 N 秒檢查一次（僅限工作時段 08:00~17:00）
        if ($enableWatchAutoServices) {
            $secondsSinceLastWatch = ($now - $global:LastServiceWatchTime).TotalSeconds
            $isWorkHour = ($now.Hour -ge 8 -and $now.Hour -lt 17)
            if ($secondsSinceLastWatch -ge $watchAutoServiceIntervalSec) {
                $global:LastServiceWatchTime = $now
                if ($isWorkHour) { 
                    Watch-AutoServices 
                } else { 
                    Write-ApiLog " -> [服務守護] 目前時間 $($now.ToString('HH:mm')) 不在工作時段 (08:00~17:00)，略過本次掃描" -Color DarkGray 
                }
            }
        }
        ...
        elseif ($path -eq "/tomcat/status") {
            try {
                ...
                try {
                    ...
                    Write-ApiLog "獲取效能數據失敗: $($_.Exception.Message)" -Color Yellow 
                }
                ...
                $out.data = @{ 
                    ...
                    "SysMemPct"=$sysMemPct; 
                    "TomcatCpu"=$tomcatCpuPct; 
                    "TomcatMemUsed"=$tomcatMemBytes; 
        ...
        elseif ($path -eq "/tomcat/logs") {
            ...
            if ($targetFile -and (Test-Path $targetFile)) {
                ...
                try {
                    ...
                    $out.data = $logData; $out.success = $true
                } catch { 
        ...
        elseif ($path -eq "/tomcat/download-logs") {
            try {
                ...
                if ($filesToZip.Count -gt 0) {
                    ...
                    try {
                        while (($read = $fileStream.Read($buffer, 0, $buffer.Length)) -gt 0) { $res.OutputStream.Write($buffer, 0, $read) }
                        $handledBinary = $true
                ...
                if ($null -ne $stagingDir -and (Test-Path $stagingDir)) { Remove-Item -Path $stagingDir -Recurse -Force -ErrorAction SilentlyContinue }
            }
        }
